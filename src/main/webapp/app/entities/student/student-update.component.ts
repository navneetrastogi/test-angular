    ...
    ngOnInit() {
        ...
        this.studentProfileService.query({ filter: 'student-is-null' }).subscribe(
            (res: HttpResponse<IStudentProfile[]>) => {
                if (!this.student.studentProfile || !this.student.studentProfile.id) {
                    this.studentprofiles = res.body;
                } else {
                    this.studentProfileService.find(this.student.studentProfile.id).subscribe(
                        (subRes: HttpResponse<IStudentProfile>) => {
                            this.studentprofiles = [subRes.body].concat(res.body);
                        },
